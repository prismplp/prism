...
int pc_mult_ln_3(void) {
	double x = bpx_get_float(bpx_get_call_arg(1,3));
	double y = bpx_get_float(bpx_get_call_arg(2,3));
	TERM t = bpx_build_float(x * log(y));

	return bpx_unify(bpx_get_call_arg(3,3),t);
}
...
static void xsleep(unsigned int milliseconds) {
#ifndef _MSC_VER
