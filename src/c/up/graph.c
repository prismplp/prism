...
static void alloc_switch_instance_table(void)
{
    int i;
    sw_ins_tab_size = 0;
    switch_instances = (SW_INS_PTR *)MALLOC(max_sw_ins_tab_size *
                                            sizeof(SW_INS_PTR));
...
void alloc_sorted_egraph(int n)
{
    ...
    sorted_expl_graph = (EG_NODE_PTR *)MALLOC(sizeof(EG_NODE_PTR) *
...
/*
 * Export probabilities of switches from Prolog to C. A switch takes the
 * form:
 *
...
static void switch_stats(int stats[2])
{
    int num_sw_ins,i;
    SW_INS_PTR ptr;
    ...
    stats[0] = occ_switch_tab_size;
    stats[1] = num_sw_ins;
}

int pc_import_switch_stats_2(void)
{
    int stats[2];
