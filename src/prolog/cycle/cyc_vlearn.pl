cyc_vlearn_p :-
    $pp_cyc_vlearn_main(ml).
cyc_vlearn_p(Goals) :-
    $pp_cyc_vlearn_main(ml,Goals).
cyc_vlearn_h :-
    $pp_cyc_vlearn_main(vb).
cyc_vlearn_h(Goals) :-
    $pp_cyc_vlearn_main(vb,Goals).
cyc_vlearn_b :-
    $pp_cyc_vlearn_main(both).
cyc_vlearn_b(Goals) :-
    $pp_cyc_vlearn_main(both,Goals).

%% for the parallel version
$pp_cyc_vlearn_main(Mode) :- call($pp_cyc_vlearn_core(Mode)).
$pp_cyc_vlearn_main(Mode,Goals) :- call($pp_cyc_vlearn_core(Mode,Goals)).

$pp_cyc_vlearn_core(Mode) :-
    $pp_learn_data_file(FileName),
    load_clauses(FileName,Goals,[]),!,
    $pp_cyc_vlearn_core(Mode,Goals).

$pp_cyc_vlearn_core(Mode,Goals) :-
    $pp_cyc_vlearn_check_goals(Goals),
...
$pp_cyc_vlearn_check_goals(Goals) :-
    $pp_require_observed_data(Goals,$msg(1302),$pp_cyc_vlearn_core/1),
    $pp_cyc_vlearn_check_goals1(Goals).

$pp_cyc_vlearn_check_goals1([]).
$pp_cyc_vlearn_check_goals1([G0|Gs]) :-
    ( (G0 = goal(G,Count) ; G0 = count(G,Count) ; G0 = (Count times G) ) ->
        $pp_require_positive_integer(Count,$msg(1306),$pp_cyc_vlearn_core/1)
    ; G = G0
    ),
