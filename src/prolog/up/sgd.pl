sgd_learn :-
    $pp_sgd_learn_main(ml).
sgd_learn(Goals) :-
    $pp_sgd_learn_main(ml,Goals).

%% for the parallel version
$pp_sgd_learn_main(Mode) :- call($pp_sgd_learn_core(Mode)).
$pp_sgd_learn_main(Mode,Goals) :- call($pp_sgd_learn_core(Mode,Goals)).

$pp_sgd_learn_core(Mode) :-
    $pp_learn_data_file(FileName),
    load_clauses(FileName,Goals,[]),!,
    $pp_sgd_learn_core(Mode,Goals).

$pp_sgd_learn_core(Mode,Goals) :-
    $pp_sgd_learn_check_goals(Goals),
    ...
    $pp_assert_sgd_learn_stats(Mode,Output,NSwitches,NSwVals,TableSpace,
                            Start,End,StartExpl,EndExpl,StartVT,EndVT,1000),
    $pp_print_learn_stats_message(MsgT),
    $pp_print_learn_end_message(MsgM,Mode),!.

$pp_sgd_learn_check_goals(Goals) :-
    $pp_require_observed_data(Goals,$msg(1302),$pp_sgd_learn_core/1),
    $pp_sgd_learn_check_goals1(Goals).

$pp_sgd_learn_check_goals1([]).
$pp_sgd_learn_check_goals1([G0|Gs]) :-
    ( (G0 = goal(G,Count) ; G0 = count(G,Count) ; G0 = (Count times G) ) ->
        $pp_require_positive_integer(Count,$msg(1306),$pp_sgd_learn_core/1)
    ; G = G0
    ),
    $pp_require_tabled_probabilistic_atom(G,$msg(1303),$pp_sgd_learn_core/1),!,
    $pp_learn_check_goals1(Gs).

$pp_sgd(ml,Output) :-
	Debug=0,
    $pc_sgd_learn(Iterate,LogPost,LogLike,ModeSmooth,BIC,CS,Debug),
