...
for filename in args.train:
	for line in open(filename):
		...
		if (rel,e2) not in train_data:
			train_data[(rel,e2)]=[e1]
		else:
			train_data[(rel,e2)].append(e1)
...
for filename in args.test:
	for line in open(filename):
		...
		e1=ent_mapping[arr[0]]
		e2=ent_mapping[arr[2]]
		if (rel,e2) not in valid_data:
			valid_data[(rel,e2)]=[e1]
		else:
			valid_data[(rel,e2)].append(e1)
