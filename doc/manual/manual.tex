...
(possible values:\ {\tt hinge}, {\tt square} and {\tt exp}, default:\ {\tt hinge})
...
The following three well-known pairwise loss functions $f(g_1, g_2)$ exist.
...
%additional memory for $r$ is the same size with that of $w$ 
	\item[Adadelta]
	%Update steps using Adadelta performs the following calculation.
	The following calculation is performed to update a parameter vector $\vw$.
\begin{align}
r &\leftarrow \gamma r + (1 - \gamma) || \nabla_{\vw} J ||^2 \nonumber \\
\Delta \bvec{w} &= \frac{\sqrt{s + \epsilon}}{\sqrt{r + \epsilon}} \nabla_{\vw} J \nonumber \\
s &\leftarrow \gamma s + (1 - \gamma) ||\Delta \bvec{w} ||^{2} \nonumber \\
\vw &\leftarrow \vw - \Delta \bvec{w} \label{eq:adadelta}
\end{align}
The initial values of $s$ and $r$ are zero.
$\gamma$ and $\epsilon$ are parameters of the Adadelta.
%???
%Although $\alpha$ is fixed at one in the original AdaDelta, it is genera one
	\item[Adam]
	%Update steps using AdaGrad performs the following calculation for each element $w$ in the parameter vector $\vw$.
	The following calculation is performed to update each element $w$ in the parameter vector $\vw$.
	\begin{align}
	...
	\end{align}	
	where $\odot$ is an operator for element-wise products.
