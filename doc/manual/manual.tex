...
\hypersetup{pdftitle={PRISM User's Manual (Version 2.0.3)}}
\hypersetup{pdfauthor={Taisuke Sato, Neng-Fa Zhou, Yoshitaka Kameya and Yusuke Izumi}}
...
{\Large (Version 2.0.3)}\par\vspace*{12cm}\par

{\normalsize Taisuke Sato*, Neng-Fa Zhou**, Yoshitaka Kameya* and Yusuke Izumi*}

{\normalsize * Tokyo Institute of Technology} \\
{\normalsize ** CUNY Brooklyn College} \\
{\small Copyright \copyright~2012 Taisuke Sato, Neng-Fa Zhou,\\
...
This software gratefully uses several free software packages,
including public-domain modules used in B-Prolog, Mersenne Twister
(\url{http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html})
and SPECFUN (\url{http://www.netlib.org/specfun/}, we implement C version
of the digamma and the log-gamma functions based on the code of SPECFUN).
...
No.~17700140, No.~20240016 and No.~20300053) from Ministry of Education,
...
\item
  Chapter~\ref{chap:vb} explains how to use the utility for
  variational Bayesian learning with some introductory description.
\item
  Chapter~\ref{chap:parallel} explains how to use the utility for
...
have the facilities newly introduced since version~1.11, and the authors
expect these chapters to be referred to (only) by the users who are
interested in these extended facilities.  Note that `2.0' is also referred
to as a generic number of the versions numbered as 2.0.$x$, so if there
is no proviso, all descriptions about version 2.0 apply to versions 2.0.$x$.

\newpage

\section*{Major changes from version 2.0.2}
\label{change:2.0.2}

\begin{itemize}
\item
  There are no changes except the version number.
\end{itemize}

\section*{Major changes from version 2.0.1}
\label{change:2.0.1}

\begin{itemize}
\item
  \secref{sec:system:error} was modified to add a description on
  recommended operations when some exception arises.
\item
  \secref{sec:built-in:underflow} was modified to add a description on
  a straightforward workaround to avoid the side-effect from MAP estimation
  in log-scaled probability computation.
\item
  \secref{sec:built-in:flags} was modified to add a description
  on {\tt reset\_prism\_flags/0}, a built-in for resetting all flags to
  their default values.
\item
  \secref{sec:built-in:io} was reorganized, and a description on
  {\tt save\_csv/2-3}, a new built-in predicate for saving the data into
  a file in the CSV format, was added to \secref{sec:built-in:io:csv}.
\end{itemize}

\section*{Major changes from version 2.0}
\label{change:2.0}

\begin{itemize}
\item
  \secref{sec:built-in:switch:term} and
  \secref{sec:built-in:switch:set_sw} were modified to add a description
  on the case of specifying `{\tt noisy\_u}'.
\item
  \secref{sec:built-in:switch:b_msw} was added to describe the built-in
  predicates {\tt soft\_msw/2} and {\tt b\_msw/2} for backtrackable
  sampling execution of random switches.
\item
  The description on the {\tt force\_gc} flag was added into
  \secref{sec:built-in:flags:list}.
\end{itemize}
...
The clause {\tt  values(coin,[head,tail])}\predindex{values/2}
...
\begin{verbatim}
% prism
PRISM 2.0.3, (C) Sato Lab, Tokyo Institute of Technology, March, 2012
...
$P(\theta\mid D,M)\propto P(D\mid M,\theta)P(\theta)$, where $D$ is the
observed data, i.e.\ a multiset of observed goals $G_1$, $G_2$, \ldots,
$G_T$, and $M$ is the model written as a PRISM program.
...
$P^\ast(\theta\mid D,M)$ and we can make inferences based on
some averaged quantities with respect to $P^\ast(\theta\mid D,M)$, instead of
...
Then, VB learning is easily conducted by setting `{\tt hparams}' to
...
\item{\it Parameter learning}\/ (\secref{sec:built-in:learn}):\\
  ...
  Given a bag of observed goals $\{G_1,G_2,\ldots,G_T\}$ of
  probabilistic predicates (i.e.\ training data), get the
  parameters $\theta$ of switches which maximizes
  the likelihood $\prod_t P_\theta(G_t)$.
\end{description}
...
{\tt genotype(a,a)}, {\tt genotype(a,o)}, \ldots, and {\tt genotype(b,a)}
...
whose left hand side is $G'$ is called a {\em sub-explanation}\conindex{sub-explanation}\conindex{explanation!sub- ---}
...
\secref{sec:lang:model:failure}.  In addition, these conditions do not
always have to be satisfied jointly.  For example, Viterbi computation
(\secref{sec:lang:inference}) does not require the exclusiveness condition.
\conindex{exclusiveness condition}
...
(see \secref{sec:built-in:learn:learn} or \secref{sec:built-in:flags:list}).
...
During versions 1.9~--~1.12,
{\tt values\_x/2-3} had been used as an extended form of {\tt values/2}.
In version 2.0, on the other hand, multi-valued switch declarations were
redesigned to import the flexibility of {\tt values\_x/2-3}.
For backward compatibility, {\tt values\_x/2-3} are still accepted in
version 2.0, but are recommended to be renamed to {\tt values/2-3},
respectively.
...
Inside the system, while the program loaded, {\tt values/2} including
{\em ground} range specifications will be translated to {\tt values/2} with the
corresponding expanded values, like above, by the built-in
{\tt expand\_values/2}
...
\begin{enumerate}
\item
  Download the package \verb+prism203_win.zip+.
\item
...
A single united package \verb+prism203_linux.tar.gz+ is provided
...
\begin{enumerate}
\item
  Download the package \verb+prism203_linux.tar.gz+ into your home directory.
\item
...
\end{enumerate}
Internally, the package contains both binaries for 32-bit and 64-bit systems.
...
The package \verb+prism203_macx.tar.gz+ is provided for Mac OS X and contains a universal binary for PowerPC and Intel processors.
...
in explanatin search\conindex{explanation search}, since in the current
...
program, {\tt upprism} will try to run {\em only} {\tt prism\_main/1}.
...
\item {\tt expand\_probs({\it Dist},{\it N},{\it Probs})}
  ...
  \begin{itemize}
    \item
      ...
      probabilities separated by `{\tt +}' (e.g.\ {\tt 0.1+0.5+0.4}),
      ...
      {\tt expand\_probs({\it Dist},{\it Probs})}\predindex{expand\_probs/2}
      instead.  For example,
      {\tt ?- expand\_probs(0.1+\lb 0.5+\lb 0.4,Ps)} returns
      {\tt Ps = [0.1,\lb 0.5,\lb 0.4]}.
    \item
...
\item {\tt expand\_pseudo\_counts({\it Spec},{\it N},{\it Counts})}
  ...
  \begin{itemize}
    ...
    \item
      If {\it Spec} takes the form {\tt uniform($\delta$)},\pcindex{uniform}
      where $\delta$ is a non-negative number,
      the system returns a list of size $N$ whose elements are all $\delta/N$.
      Note that $N$ is mandatory here.
      If {\it Spec} is an atom {\tt uniform}, it will be interpreted
      as {\tt uniform(1.0)}.
      For example,
      {\tt ?- expand\_\lb pseudo\_\lb counts(uniform(5),\lb 4,\lb Cs)}
      returns {\tt Cs = [1.25,\lb 1.25,\lb 1.25,\lb 1.25]} and
      {\tt ?- expand\_\lb pseudo\_\lb counts(uniform,\lb 4,\lb Cs)}
    ...
    \item
      ...
      the system returns a list of size $N$ whose $i$-th element is
      $\delta\beta^{i-1}$, where we let $\beta=\mathit{Base}$.
      Here $N$ is mandatory,
      ...
      {\tt f\_\lb geometric($\delta$,\lb {\it Base})} are
      interpreted as {\tt f\_\lb geometric(1.0,\lb 2.0,\lb desc)},
      {\tt f\_\lb geometric(1.0,\lb {\it Base},\lb desc)} and
      {\tt f\_\lb geometric($\delta$,\lb {\it Base},\lb desc)},
      respectively.
    \item
      ...
      is set to {\tt uniform},
      {\tt ?- expand\_pseudo\_counts(\lb default,\lb 4,\lb Ps)} returns
      {\tt Ps = [0.25,\lb 0.25,\lb 0.25,\lb 0.25]}.
  \end{itemize}
\end{itemize}
%
...
\item
  ...
  be kept unchanged during VB learning (\secref{sec:vb:built-in:learn}).
...
{\tt unfix\_sw\_a/1}.  Also, for backward compatibility,
...
$\alpha_{i,v}$''.  For backward compatibility,
{\tt show\_sw\_h/0-1} and {\tt show\_sw\_b/0-1}
are available as the aliases of
{\tt show\_sw\_d/0-1} and {\tt show\_sw\_pd/0-1}, respectively.
...
\item
  ...
  \predindex{get\_sw/1}  
  binds {\it Info} to a Prolog term in the form
  {\tt switch({\it I},{\it Status},{\it Vals},{\it Params})}
...
\item
  ...
  binds {\it Info} to a Prolog term in the form
  {\tt switch({\it I},[{\it StatusP},{\it StatusH}],{\it Vals},\lb {\it Params},{\it PseudoCs})}
...
\item
  ...
  additionally returns the expected occurrences\conindex{expected occurrence}
  {\it \~{C}s} of switch {\it I},
  which are computed in VB-EM learning (\secref{sec:vb:bg:learn}).
...
are still available as the aliases of {\tt save\_sw\_d/0-1},
...
The built-in {\tt get\_samples\_c({\it N}\/,{\it G}\/,{\it C}\/,{\it Gs}\/,[{\it SN}\/,{\it FN}\/])}
\predindex{get\_samples\_c/5} behaves similarly to
{\tt get\_\lb samples\_\lb c({\it N},{\it G},{\it C},{\it Gs})},
...
with the ones specified in {\it Options}.  {\it Options} can contain
...
\subsection{Maximum likelihood estimation and EM learning}
...
in ML estimation, the system tries to find the parameters $\theta$ that
maximize the likelihood\conindex{likelihood}
$\prod_t P_\theta(G_t)$, the product of probabilities
...
\item{\it Initialization step}\/:\\
  ...
  two steps until the likelihood converges.
...
\item{\it Maximization step}\/:\\
  ...
  $\hat{\theta}^{(m+1)}_{i,v}=\hat{C}_{i,v}/\sum_{v'}\hat{C}_{i,v'}$
...
probability given the parameters.
}
...
\subsection{Running learning commands}
\label{sec:built-in:learn:learn}
...
which is the substantial target of maximization.  Finally we find the
...
It is only guaranteed by the EM algorithm\conindex{expectation-maximization algorithm}
...
multiple runs of the EM algorithm by restarting\conindex{restart}
...
each temperature $1/\beta$.  Fig.~\ref{fig:daem} shows an
...
{\tt num\_iterations}\statindex{num\_iterations}&Number of EM iterations in the last learning\\
...
{\tt em\_time}\statindex{em\_time}&Time consumed by the EM algorithm (in seconds)\\
\hline
...
and {\tt infer\_\lb statistics/0}\predindex{infer\_statistics/0} display
the statistics on the explanation graphs, on learning, and on
the probabilistic inferences other than learning.
{\tt prism\_\lb statistics/0}\predindex{prism\_statistics/0}
displays all statistics displayed by the above three built-ins.  
...
{\tt graph\_\lb statistics/2}, {\tt learn\_\lb statistics/2} and
{\tt infer\_\lb statistics/2}). The available statistics are
...
candidates.  In PRISM, the programming system provides three Bayesian
scores\conindex{Bayesian score} called {\em Bayesian Information Criterion}
(BIC)~\cite{Schwarz78}, \conindex{Bayesian Information Criterion}
\conindex{BIC|see{Bayesian Information Criterion}}
the {\em Cheeseman-Stutz (CS) score}~\cite{Cheeseman95}
\conindex{Cheeseman-Stutz score}\conindex{CS score|see{Cheeseman-Stutz score}}
and {\em variational (negative) free energy}\conindex{variational free energy}.
The first two are used after ML (\secref{sec:built-in:learn:mle}) or MAP
(\secref{sec:built-in:learn:mape}) estimation, whereas the last one is
used after variational Bayesian learning (Chapter~\ref{chap:vb}).
Generally speaking, these Bayesian scores are known to be `deterministic'
approximations of $\log P(D\mid M)$, log of the {\em marginal likelihood}
\conindex{marginal likelihood} of the observed data $D$ under the model $M$,
...
distribution $p(D,M,\theta)$ of the observed data $D$, a probabilistic
model $M$, and its parameters $\theta$.  In PRISM, $D$ is a multiset
of observed goals $G_1,G_2,\ldots,G_T$, and $M$ corresponds to
the modeling part\conindex{modeling part} of a PRISM program.
$p(D,M,\theta)$ is then factored as
$p(D\mid M,\theta)p(\theta\mid M)p(M)$ by the chain rule, where
...
of the parameters $\theta$ of the model $M$, and $p(D\mid M,\theta)$
is the {\em likelihood}\conindex{likelihood} of the data $D$ based
...
$D$ at hand, that is, we attempt to find $M^\ast$ such that:
\[
M^\ast
=\mathrm{argmax}_M\; p(M\mid D)
=\mathrm{argmax}_M\; \frac{p(D\mid M)p(M)}{p(D)}
=\mathrm{argmax}_M\; p(D\mid M),
\]
where we assume $p(M)$ to be uniform for simplicity.  Now the goal
is reduced to finding $M\;(=M^\ast)$ that maximizes $p(D\mid M)$.
$p(D\mid M)$ is commonly called the {\em marginal likelihood}
\conindex{marginal likelihood} of $D$ given $M$, and is used as a
...
$p(D\mid M,\theta)$ with respect to the prior distribution $p(\theta\mid M)$:
\[
p(D\mid M)
=\textstyle \int_{\Theta} p(D,\theta\mid M) d\theta
=\textstyle \int_{\Theta} p(D\mid M,\theta)p(\theta\mid M) d\theta
=\langle p(D\mid M,\theta)\rangle_{p(\theta\mid M)}~.
\]
If the observed data were complete data $D_c$, where each $d\in D_c$
is a pair $(G_t,E_t)$ of the $t$-th goal $G_t$ and its {\em unique} explanation
$E_t$, then $p(D_c\mid M)$ is obtained in closed form
...
&\defined&p(D\mid M,\hat{\theta}_{\mathrm{MAP}})-\frac{|\theta|}{2}\log N\\
\mathit{Score}_{\mathrm{CS}}(M)
&\defined&p(\tilde{D}_c\mid M)-p(\tilde{D}_c\mid M,\hat{\theta}_{\mathrm{MAP}})
+p(D\mid M,\hat{\theta}_{\mathrm{MAP}}),
\end{eqnarray*}
where $N$ is the total size of dataset, $|\theta|$ denotes the number
of free parameters, $\hat{\theta}_{\mathrm{MAP}}$ is the MAP estimate
of the parameters, and $\tilde{D}_c$ is the pseudo complete data whose
...
about BIC and the CS score.  The definition of the variational free energy
will be shown in Chapter~\ref{chap:vb}.
...
MAP learning (\secref{sec:built-in:learn:learn}) with some observed goals $D$.
...
\item
  ...
  default:\ {\tt data/1})
  ---
  the data file for {\tt learn/0} (\secref{sec:built-in:learn:learn}).
...
\item
  ...
  {\tt f\_\lb geometric({\it Base})}, {\tt f\_\lb geometric({\it Base},{\it Type})},
  ...
  {\tt f\_geometric({\it Base},{\it Type})} means the default distribution
...
\item
  ...
  learning (\secref{sec:vb:built-in:learn}).  {\tt uniform}
  (resp.\ {\tt uniform($\zeta$)}) means that each pseudo count will
...
\item
  ...
  \item The message on the progress of the EM algorithm
...
\item
  {\tt learn\_mode}
  \flagindex{learn\_mode}
  (possible values:\ {\tt params}, {\tt hparams} and {\tt both}; default:\ {\tt params})
  ---
  the underlying statistical framework for parameter learning.
  If this flag is set as `{\tt params}', the system will conduct the EM algorithm
  for ML/MAP estimation\conindex{maximum likelihood estimation}
  \conindex{maximum a posteriori estimation}
  (\secref{sec:built-in:learn}), by which we can get point-estimated
  parameters\conindex{parameter} of random switches.
  If this flag is set as `{\tt hparams}', the system
  will conduct the EM algorithm for VB learning\conindex{variational Bayesian learning}
  (\secref{sec:vb:built-in:learn}),
...
\item
  ...
  in the repeated runs of VB learning\conindex{variational Bayesian learning!repeated runs of ---}
  (\secref{sec:vb:built-in:learn}).
  ...
  VB learning.  If this flag is set as `{\tt off}', on the other hand, it can
  be observed that the pseudo counts monotonically increases as we repeatedly
...
\item
  ...
  (possible values:\ {\tt params} and {\tt hparams}; default:\ {\tt params})
  ---
  the underlying statistical framework for Viterbi computation.
  If this flag is set as `{\tt params}', the system will conduct the Viterbi
  computation based on the current parameter values (\secref{sec:built-in:viterbi}).
  If `{\tt hparams}' is set, on the other hand, the system
...
These syntactic constracts are compiled at loading time, whereas the current
...
\begin{itemize}
  ...
  \item {\tt maplist\_func({\it F},{\it Xs},{\it Ys})}\predindex{maplist\_func/3}
    ...
    succeeds for every pair of corresponding elements in {\it Xs} and {\it Ys}.
    This is equivalent to {\tt maplist(X,Y,{\it F}(X,Y),{\it Xs},{\it Ys})}.
  ...
  \item {\tt reducelist\_math({\it Op},{\it List},{\it Init},{\it Value})}\predindex{reducelist\_math/4}
    ...
    This is equivalent to {\tt reducelist(Y0,X,Y1,(Y1 is {\it Op}(Y0,X)),{\it List},{\it Init},{\it Value})},
    but is more efficient.
\end{itemize}
...
\item
  ...
  \item{$\diamond$~Other options}:
    ...
    \item
      ...
      or if we specify {\tt parse\_number(yes)}, a value ``{\tt 123456}'' in
...
In this section, we briefly describe the background about the VB approach.
The paper \cite{Sato09} gives a full description on this topic with
some applications.

\subsection{VB-EM learning}
\label{sec:vb:bg:learn}

As mentioned in \secref{sec:overview:vb}, variational Bayesian (VB)
learning has high robustness against data sparseness\conindex{data sparseness}
in model selection
\conindex{model selection} and prediction (Viterbi computation).
\conindex{Viterbi computation}
For model selection, an introductory description in Bayesian approaches
is given in \secref{sec:built-in:learn:score}.  To choose the best model (the best
PRISM program) $M^\ast$ that fits best the data $D$ at hand, we consider
$M=M^\ast$ is the model that maximizes the marginal likelihood
\conindex{marginal likelihood} $p(D\mid M)$. It has been also known that
if $D$ is complete data\conindex{complete data} $D_c$, $p(D\mid M)$
can be obtained in closed form.  However, when $D$ is incomplete,
\conindex{incomplete data} i.e.\ there is some hidden data $\vz$ such
that $D_c=(D,\vz)$ (in PRISM, $\vz$ corresponds to a hidden explanation
for the observed goals), some approximation is required.

First, let us consider log of the marginal likelihood\conindex{marginal likelihood}
$L(D)\defined\log p(D\mid M)$, and then we have:
\begin{eqnarray*}
L(D)
&=&\log\sum_{\vz}\int_{\Theta} p(D,\vz,\theta\mid M) d\theta\\
&=&\log\sum_{\vz}\int_{\Theta}
     q(\vz,\theta\mid D,M)
     \frac{p(D,\vz,\theta\mid M)}{q(\vz,\theta\mid D,M)}
     d\theta\\
&\ge&\sum_{\vz}\int_{\Theta}
       q(\vz,\theta\mid D,M)
       \log\frac{p(D,\vz,\theta\mid M)}{q(\vz,\theta\mid D,M)}
       d\theta. \q\q\q\bigl(\mbox{from Jensen's inequality}\bigr)
\end{eqnarray*}
For the space limitation, we fix the model $M$ for the moment,
and simply write $p(\cdot\mid M)=p(\cdot)$ and
$q(\cdot\mid D,M)=q(\cdot\mid D)$, and then obtain:
\[
L(D)\ge F[q]\defined\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)
            \log\frac{p(D,\vz,\theta)}{q(\vz,\theta\mid D)} d\theta
\]
where $F[q]$ can be seen as a lower limit of $L(D)$, and is called the
{\em variational free energy}.\conindex{variational free energy}
\conindex{free energy!variational ---|see{variational free energy}}
So to get a good approximation of $L(D)$,
...
$L(D)=\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)\log p(D)d\theta$:
\begin{eqnarray*}
L(D)-F[q]
&=&\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)
     \log\left\{
           p(D)\cdot\frac{q(\vz,\theta\mid D)}{p(D,\vz,\theta)}
         \right\}d\theta\nonumber\\
&=&\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)
     \log\frac{q(\vz,\theta\mid D)}{p(\vz,\theta\mid D)}d\theta
\;=\;\kldiv{q(\vz,\theta\mid D)}{p(\vz,\theta\mid D)}.
\end{eqnarray*}
From the above, maximizing $F[q]$ implies minimizing the Kullback-Liebler
divergence between $q(\vz,\theta\mid D)$ and $p(\vz,\theta\mid D)$.
So finding $q^\ast$ is to make a good approximation of $p(\vz,\theta\mid D)$,
the conditional distribution of hidden variables and parameters.

In VB learning, we further assume $q(\vz,\theta\mid D)\approx q(\vz\mid D)q(\theta\mid D)$,
...
q(\vz\mid D)&\propto&
\textstyle
  \exp\left(\int_{\Theta} q(\theta\mid D)\log p(D,\vz\mid\theta) d\theta\right),\\
q(\theta\mid D)&\propto&
\textstyle
  p(\theta)\exp\left(\sum_{\vz}q(\vz\mid D)\log p(D,\vz\mid\theta)\right).
\end{eqnarray*}
Please recall that, in PRISM, $D$ is a multiset of the observed goals
$G_1,G_2,\ldots,G_T$, and that $\vz$ corresponds to a hidden explanation
for the goals.  The VB-EM algorithm for PRISM is then derived from the above
...
\item
  ...
  $\bar{\theta}_{i,v}$ of the parameters
...
\[
F[q]
=\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)\log p(D,\vz,\theta) d\theta
-\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)\log q(\vz,\theta\mid D) d\theta
\]
...
\[
F_\beta[q]
\defined\sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)\log p(D,\vz,\theta) d\theta
-\frac{1}{\beta}
   \sum_{\vz}\int_{\Theta} q(\vz,\theta\mid D)\log q(\vz,\theta\mid D) d\theta.
\]
...
\section{Built-in utilities for variational Bayesian learning}
\label{sec:vb:built-in}

\subsection{VB-EM learning}
\label{sec:vb:built-in:learn}
...
To enable VB-EM learning, we give a value `{\tt hparams}'
(which indicates that we wish to get the adjusted
hyperparameters\conindex{hyperparameter} by VB-EM)
to the {\tt learn\_mode} flag, and then run the usual
learning command ({\tt learn/0-1}) as follows:
\predindex{learn/1}
\flagindex{learn\_mode}
\begin{quote}
\begin{verbatim}
...
On the other hand, to disable VB-EM, give `{\tt params}' to the
{\tt learn\_mode}\flagindex{learn\_mode} flag
(the default value of the {\tt learn\_mode} flag is `{\tt params}').
This indicates that we wish to get the point-estimated parameters
\conindex{parameter}\conindex{parameter!point-estimated ---} of
...
framework for Viterbi computation. If we give a value `{\tt hparams}'
...
On the other hand, if we give a value `{\tt params}' to the
...
As described in \secref{sec:vb:bg:learn}, in VB-EM learning, the programming system
...
The setting of the execution flags related to variational Bayes (VB) is rather
complicated, so in this section, we will show several typical usages.  Before
...
\item 
  ...
  \begin{quote}
    \begin{verbatim}
:- set_prism_flag(learn_mode,both).
:- set_prism_flag(viterbi_mode,params).
    \end{verbatim}
  \end{quote}
  or equivalently,
  \begin{quote}
...
:- set_prism_flag(learn_mode,both).
    \end{verbatim}
  \end{quote}
  (the default value of the {\tt viterbi\_mode} flag is {\tt params}).
\item
...
(\secref{sec:built-in:learn:learn}) are used similarly
...
model, a special form of a Bayesian network (see Fig.~\ref{fig:nbayes}).  First,
...
built-in for splitting lists.  {\tt load\_data\_file/1} uses {\tt load\_csv/2}
to read a CSV file ({\tt house-votes-84.data}) directly and {\tt maplist/5}
to convert each row in the CSV file into an observed goal {\tt nbayes(C,Vs)}
in the model.
