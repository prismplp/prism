...
\hypersetup{pdftitle={PRISM User's Manual (Version 2.2 pre-release)}}
...
{\Large (Version 2.2 pre-release)}\par\vspace*{12cm}\par

{\normalsize Taisuke Sato*, Neng-Fa Zhou\dag, Yoshitaka Kameya\ddag, Yusuke Izumi*, Keiichi Kubota* and Ryosuke Kojima*}
...
{\footnotesize Copyright \copyright~2014 T. Sato, N.-F. Zhou, Y. Kameya, Y. Izumi, K. Kubota and R. Kojima}
\end{center}
\thispagestyle{empty}
...
\begin{itemize}
\item
  The descriptions on the flags related to MCMC sampling
...
\begin{enumerate}
\item
  Download the package \verb+prism21_win.zip+.
\item
...
A single united package \verb+prism21_linux.tar.gz+ is provided
...
\begin{enumerate}
\item
  Download the package \verb+prism21_linux.tar.gz+ into your home directory.
\item
...
The package \verb+prism21_macx.tar.gz+ is provided for Mac OS X and contains a universal binary for PowerPC and Intel processors.
...
generalization of {\em generative-discriminative pair\/}
...
given an input sequence $\vx$  which takes  the  following form:\footnote{
  Bold italic letters are (values of) random  vectors.
}
%
\begin{eqnarray}
p(\vy \mid \vx) 
...
(Eq.~\ref{eq:regcondlikelihood}).   We can  use  any library  for this  numerical
optimization but currently L-BFGS \cite{Liu89}\conindex{L-BFGS algorithm}
and the steepest descent algorithm\conindex{steepest descent method}
...
Viterbi  inference  for  CRFs  is  carried out  by  built-in  predicates
{\tt crf\_viterbi/1},
{\tt crf\_prob/2} and  so on.
{\tt ?-~crf\_viterbi(G)}\predindex{crf\_viterbi/1}
...
for probabilistic models  containing cyclic structure such as  loops in Markov
...
plan recognition. PRISM's  probability computation on
...
(\secref{sec:cycle:usage}).  Additionally we illustrate how to perform probability
computation using cyclic explanation graphs (\secref{sec:cycle:lin}
and \secref{sec:cycle:nonlin}).
...
To use {\tt lin\_prob({\it  G})} and {\tt nonlin\_prob({\it G})} however,
...
acyclic  or  linear  cyclic   explanation  graphs  for  which  more  efficient
computation is possible.


\section{Examples of linear cyclic explanation graph}
\label{sec:cycle:lin}

Cyclic  explanation graphs  enable us  to deal  with useful  models  of cyclic
...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.5\hsize]{fig/DTMC01.eps}
...
prefix_pcfg([A|R],L0,L2):-                % L0 is ground when called
    ( get_values(A,_) -> msw(A,RHS),      % if A is a nonterminal
        prefix_pcfg(RHS,L0,L1)            % select rule A->RHS
...
parser\footnote{
  % 
  The program is specialized for a PCFG containing CFG rules {\tt s -> ss}(0.4),
  {\tt  s -> a}(0.3)  and {\tt  s ->  b}(0.3) where  numbers in  parentheses are
  application probabilities of rules.
  % 
}
where  the only difference  is the insertion  of one line code  commented as
``{\tt (pseudo) success}.''
...
\section{Example of nonlinear cyclic explanation graphs}
\label{sec:cycle:nonlin}

{\it Nonlinear  cyclic explanation  graph\/}s enable the  user to  perform yet
...
this program constructs  a finite automaton {\tt FA} for  the input infix {\tt
  L} and takes the intersection of  {\tt FA} and a given PCFG while preserving
