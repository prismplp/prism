...
This system provides utilities to solve such ranking problem.
In the following, we first give an overview of ranking in the programming system (\secref{sec:rank:background}) and then describe the basic usage of built-in utilities related to ranking (\secref{sec:rank:util}).


\section{Background}
\label{sec:rank:background}

Ranking in this system provides two features: learning parameters from the ranked goals (learning to rank) and ranking the given goals (ranking goals).
In learning to rank, a scoring function is constructed by minimizing a loss function
on given goals.
In ranking goals, given a new list of goals, the scoring function is applied to 
produce a list sorted in descending order of their scores.
The scoring function from the given goal to the ranking score is defined as the log probability of the goal.
Given a list $G=\left[g_1, g_2, g_3, ..., g_n\right]$ of goals, The loss function $L(G)$ is defined as follows:

\[
L(G)=\sum_{i=1}^{n-1} f( g_{i}, g_{i+1} ) 
\]
\[
f(g_1,g_2) = \begin{cases}
\left(l(g_2)-l(g_1)\right)^2 & (l(g_1)<l(g_2)) \\
0 & (otherwise)
\end{cases}
\]

where
$l(g)=log P(g)$ is a scoring function of a goal $g$,
The pairwise loss function $f(g_1,g_2)$ is positive unless $l(g_1) > l(g_2)$.

This loss function is categorized in the pairwise approach.
This approach is widely used,
for example, in the loss functions of Ranking SVM \cite{joachims2002optimizing}, RankBoost \cite{freund2003efficient}, and RankNet \cite{burges2005learning}.
The loss function of this system is inspired by the loss function of ProPPR\cite{wang2013programming}.

The minimization of the loss is performed by the SGD (stochastic gradient decent) with the $L_2$ regularization.
This algorithm works well in many cases when its parameter called learning rate is appropriately set.
This system supports manual setting and several adaptive algorithms to optimize the learning rate.
...
\begin{itemize}
\item {\tt rank\_learn(GoalLists)} learns parameters from a list of ranked goal lists {\tt GoalLists}. The loss function (\secref{sec:rank:background}) is used and its optimization algorithms are switched and controlled using flags: {\tt sgd\_penalty}, {\tt sgd\_learning\_rate}, and {\tt sgd\_optimizer} (\secref{sec:built-in:flags:list}
).
\item {\tt rank(GoalList,RankedList)} returns the ranked goal list from given unsorted goal list.
\item {\tt rank(GoalList,RankedList,Scores)} returns the ranked goal list with their ranking scores from given unsorted goal list.
\end{itemize}

When you want the ranking score, {\tt prob}-family predicates (\secref{sec:built-in:prob}) are available, because this system adopt the log probability of the goal as the ranking score.
